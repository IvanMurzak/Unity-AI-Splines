...
⚠️ Make sure you done editing `package.json` and files in `Packages/com.ivanmurzak.unity.mcp.splines` folder. Because it is going to be public with no ability to discard it.

1. Increment package version in the file `Packages/com.ivanmurzak.unity.mcp.splines/package.json`. It has `version` property.
