...
> **Template doc.** This file is part of the Unity-AI-Tools-Template and uses
> placeholders (`YOUR_PACKAGE_ID`, `IvanMurzak/YOUR_REPO`). When you adopt the
> template for a real extension, replace every placeholder with your real
> package id (e.g. `com.company.package`) and repository slug.
...
This document describes how an extension built from this template signs its
...
`Unity-Package/Packages/com.ivanmurzak.unity.mcp.splines/` with Unity's UPM CLI, verifies the resulting archive
...
If OpenUPM's package listing for `YOUR_PACKAGE_ID` has `trackingMode: git`,
OpenUPM packs and serves unsigned tarballs from the repository's git tags. To make
OpenUPM serve the signed tarball that the workflow now uploads, the listing must be
...
ships a `.unitypackage` asset, and may add more `.tgz` assets later (e.g.
signing a second package), so the prefix guard prevents a future-breaking
failure mode:
